...
final class BotProcessManager: ObservableObject {
    ...
    func start() {
        ...
        guard let configStore else {
            appendLog("❌ AppConfigStore no está conectado.\n")
        ...
        guard !configStore.telegramBotToken.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            appendLog("❌ Falta Telegram Bot Token.\n")
        ...
        task.terminationHandler = { [weak self] proc in
            Task { @MainActor in
                ...
                if proc.terminationStatus == 0 {
                    ...
                    self.appendLog("\n⚠️ El proceso terminó con código \(proc.terminationStatus).\n")
                }
            }
        }
        ...
        do {
            ...
            appendLog("❌ No se pudo iniciar el bot: \(error.localizedDescription)\n")
        }
    }
    ...
    private func updateStatusFromLog(_ text: String) {
        ...
        } else if lower.contains("bot iniciado") || lower.contains("mensaje recibido") || lower.contains("respuesta enviada") {
            statusText = "Activo"
        }
    }
